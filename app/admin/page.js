...
import { Truck, Calendar, Users, Settings, ClipboardList, DollarSign, Clock, CheckCircle, AlertCircle, LogOut, Menu, X, ChevronRight, Mail, Tag } from 'lucide-react';
  ...
  const Sidebar = () => (
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-black text-white transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} md:translate-x-0 transition-transform duration-200 ease-in-out`}>
      ...
      <nav className="p-4 space-y-2">
        ...
        <Link href="/admin/discounts" className="flex items-center px-4 py-3 rounded-lg hover:bg-gray-800 text-gray-300 hover:text-white">
          <Tag className="h-5 w-5 mr-3" /> Discount Codes
        </Link>
